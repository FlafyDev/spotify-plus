...
class XpuiInjector {
  ...
  constructor(
    public xpuiPath: string,
    public vxpuiPath: string,
    ...
    this.xpuiReactElements = XpuiInjector.getReactElements(
      this.xpuiModifier.parsed["body"]
    );
  }
  ...
  injectNamedComponents() {
    ...
    while (true) {
      ...
      if (namedComponent) {
        ...
        this.assignVariableToObject(
          this.xpuiModifier,
          namedComponent,
          `${this.apiVariable}.NamedComponents['${value}']`
        );
      } else {
        break;
      }
  ...
  injectMenu() {
    const element = this.xpuiReactElements.find((reactElement) => {
      const identifiers = deepSearch(
        reactElement,
        (obj) => obj?.type === "Identifier"
      ).map((obj) => obj?.name);
      return (
        difference(
          ["children", "onClose", "getInitialFocusElement"],
          identifiers
        ).length === 0
      );
    });
  ...
  injectMenuItem() {
    const element = this.xpuiReactElements.find((reactElement) => {
      ...
      return (
        difference(
          ["children", "icon", "disabled", "divider", "onClick"],
          identifiers
        ).length === 0
      );
    });
  ...
  injectSubMenu() {
    const element = this.xpuiReactElements.find((reactElement) => {
      ...
      return (
        difference(["displayText", "depth", "children", "divider"], identifiers)
          .length === 0
      );
    });
  ...
  injectPopup() {
    ...
    this.assignVariableToObject(
      this.xpuiModifier,
      popupElement,
      `${this.apiVariable}.GenericModal`
    );
  }

  injectRightClickOpenContextMenu() {
    const element = this.xpuiReactElements.find((reactElement) => {
      const objectExpressions = deepSearch(
        reactElement,
      ...
      return (
        objectExpressions.find((objectExpression) => {
          const properties = objectExpression.properties;

          return (
            isEqual(
              ["action", "trigger"],
              properties.map((prop: any) => prop.key.name)
            ) &&
            isEqual(
              ["open", "right-click"],
              properties.map((prop: any) => prop.value.value)
            )
          );
        }) !== undefined
      );
    ...
    this.assignVariableToObject(
      this.xpuiModifier,
      element,
      `${this.apiVariable}.RightClickOpenContextMenu`
    );
  }

  injectGetShowFeedback() {
    const variableDeclarator = deepSearch(
      this.xpuiModifier.parsed["body"],
      (obj) => {
        obj?.type === "VariableDeclarator" &&
          obj?.init?.type === "SequenceExpression";
        const objectExpressions = deepSearch(
          obj.init?.expressions?.[1],
          (obj) => obj?.type === "ObjectExpression",
          10
        );
    ...
    this.assignVariableToObject(
      this.xpuiModifier,
      variableDeclarator,
      `${this.apiVariable}.getShowFeedback`
    );
  }

  injectPlatform() {
    const somePlatformProps = [
      "version",
      "container",
      "operatingSystem",
      "isDeveloperMode",
    ...
    const platformObjectExpression = deepSearch(
      this.xpuiModifier.parsed["body"],
      (obj) => obj?.type === "ObjectExpression"
    ).find((obj) => {
  ...
  injectReact() {
    ...
    // Looks for an object that has all of react's properties.
    const reactObjectExpression = deepSearch(
      this.vxpuiModifier.parsed["body"],
      (obj) => obj?.type === "ObjectExpression"
    ).find((obj) => {
      return isEqual(
        obj.properties.map((prop: any) => prop.key.name),
        reactProps
      );
    });
    ...
    this.assignVariableToObject(
      this.vxpuiModifier,
      reactObjectExpression,
      `${this.apiVariable}.React`
    );
  }
  ...
  injectReactDOM() {
    ...
    const reactDOMObjectExpression = deepSearch(
      this.vxpuiModifier.parsed["body"],
      (obj) => obj?.type === "ObjectExpression"
    ).find((obj) => {
      return isEqual(
    ...
    this.assignVariableToObject(
      this.vxpuiModifier,
      reactDOMObjectExpression,
      `${this.apiVariable}.ReactDOM`
    );
  }

  async injectSPInit() {
    const spApiInitializer = (
      await fs.readFile(path.join(__dirname, "../../files/SPApiInitializer.js"))
    ).toString();
    this.xpuiModifier.insert(0, 0, spApiInitializer);
  }

  private assignVariableToObject(
    jsModifier: JSModifyManager,
    obj: any,
    variable: string
  ) {
    switch (obj.type) {
  ...
  private static getReactElements(parsedBody: any) {
    return deepSearch(parsedBody, (obj) => {
      if (obj?.type === "VariableDeclarator" || obj?.type === "Property") {
        ...
        switch (obj.type) {
          ...
          case "Property": {
            blockStatement =
              obj?.value?.body || obj?.value?.arguments?.[0]?.body;
            break;
          }
        }
        ...
        if (
          ...
          blockStatement.body[blockStatement.body.length - 1]?.type !==
            "ReturnStatement"
        ) {
          return false;
        }
